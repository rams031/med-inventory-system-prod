...
function Login() {
  ...
  const loginAction = async (e) => {
    ...
    await postApiData("/account/login", { username, password })
      .then(({ status, data }) => {
        if (status === 200) {
          if (data.length === 0) {
            setShowLabel(true);
    ...
    await postApiData("/account/create", registerFormValues)
      .then(({ status }) => {
        if (status === 200) {
          ...
          setUserView("login");
          setRegisterFormValues(registerObject);
          return setShowLoading(false);
        }
      })
      .catch((err) => {
  ...
  const loginDisplay = () => {
    return userView === "login" ? (
      ...
      >
        ...
        {showLabel && (
          <div className="bg-red-200 text-red-600 text-md font-semibold text-center rounded-lg py-1">
            Invalid Credentials
          </div>
        )}
        <div>
        ...
        <div className="flex flex-row justify-end items-center pt-2">
          ...
          {showLoading && (
            <svg
              aria-hidden="true"
              class="w-8 h-8 mr-2 text-yellow-200 bg-white animate-spin fill-yellow-600"
              viewBox="0 0 100 101"
              fill="none"
